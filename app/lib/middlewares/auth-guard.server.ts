...
export const authMiddleware: unstable_MiddlewareFunction = async ({ request, context }, next) => {
...
export const noAuthMiddleware: unstable_MiddlewareFunction = async ({ request }, next) => {
  const authSession = await getAuthSession(request);
