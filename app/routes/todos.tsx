...
import { type SelectTodo, todo } from "~/lib/database/schema-identity";
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  switch (submission.value.intent) {
    case "create":
      ...
      break;
    case "delete":
      await db.delete(todo).where(and(eq(todo.id, Number(submission.value.id)), eq(todo.userId, authSession.user.id)));
      break;
    case "toggle":
      await db
        .update(todo)
        ...
        .where(and(eq(todo.id, Number(submission.value.id)), eq(todo.userId, authSession.user.id)));
      break;
  }
...
export default function TodosRoute({ loaderData: { todos }, actionData }: Route.ComponentProps) {
  ...
  const isSubmitting = navigation.state !== "idle" && navigation.formData?.get("intent") === "create";

  return (
    <div className="mx-auto max-w-3xl space-y-6 p-4 sm:p-10">
      <div className="flex items-center justify-between">
        <h1 className="font-semibold text-base">Todo List</h1>
        <div className="font-mono text-muted-foreground text-sm">Today is {formatDate(new Date(), "MMMM d, yyyy")}</div>
      </div>

      <Form method="POST" className="space-y-2" {...getFormProps(form)}>
        <div className="flex gap-2">
          <Input placeholder="Add a todo" {...getInputProps(fields.title, { type: "text" })} />
          <input defaultValue="create" {...getInputProps(fields.intent, { type: "hidden" })} />
          <LoadingButton buttonText="Add" loadingText="Adding..." isPending={isSubmitting} />
        </div>
        {fields.title.errors && (
