...
export default function ConnectionsRoute({ loaderData: { accounts } }: Route.ComponentProps) {
  const connections = SOCIAL_PROVIDER_CONFIGS.map((config) => {
    ...
    return {
      ...
      icon: config.icon,
      isConnected: !!account,
      createdAt: account?.createdAt ? formatDate(account.createdAt, "MMM d, yyyy hh:mm a") : null,
    };
  });

  return (
    <SettingsLayout title="Connections" description="You can connect your account to third-party services below.">
      <div className="py-4">
