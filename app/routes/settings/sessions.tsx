...
export default function SessionsRoute(_: Route.ComponentProps) {
  ...
  return (
    ...
    >
      <div className="py-4">
        ...
        >
          <Await
            resolve={_.loaderData.listSessions}
            errorElement={
                ...
                <div className="divide-y rounded-lg border shadow-xs">
                  {resolvedSessions.length === 0 ? (
                    ...
                    resolvedSessions.map((item) => (
                      <SessionItem key={item.token} session={item} currentSessionToken={session.token} />
                    ))
                  )}
                </div>
