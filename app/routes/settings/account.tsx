  ...
  try {
    ...
    switch (intent) {
      ...
      case "set-avatar": {
        ...
        await Promise.all([
          cloudflare.env.CF_R2.put(objectName, image, {
  ...
  return (
    <SettingsLayout title="Account">
      <SettingRow
        ...
        action={<AvatarCropper avatarUrl={avatarUrl} placeholderUrl={placeholderUrl} />}
      />
      <SettingRow title="Name & Email address" description={`${user.name}, ${user.email}`} />
      <SettingRow title="Current sign in" description={`You are signed in as ${user.email}`} action={<SignOut />} />
      <SettingRow
