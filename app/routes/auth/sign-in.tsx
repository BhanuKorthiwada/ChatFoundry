...
export default function SignInRoute() {
  ...
  const isPending = (provider: string) =>
    navigation.formData?.get("provider") === provider && navigation.state !== "idle";
  const isSignInPending = isPending("sign-in");

  return (
    <AuthLayout title="Sign in to your account" description="Welcome back! Please sign in to continue.">
      {/* Sign in form */}
      <Form method="post" className="grid gap-4" {...getFormProps(form)}>
        ...
        <PasswordField
          labelProps={{
            ...
            children: (
              <>
                <span>Password</span>
                <Link to="/auth/forget-password" className="font-normal text-muted-foreground hover:underline">
        ...
        <LoadingButton buttonText="Sign In" loadingText="Signing in..." isPending={isSignInPending} />
      </Form>
        ...
        <div className="grid gap-2">
          {SOCIAL_PROVIDER_CONFIGS.map((config) => (
            <Form key={config.id} method="post">
              ...
              <Button variant="outline" className="w-full" disabled={isPending(config.id)}>
