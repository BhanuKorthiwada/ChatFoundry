...
const ERROR_STATUS_MAP: Record<number, { message: string; defaultDetails: string }> = {
  400: {
  ...
  401: {
    ...
    defaultDetails: "Please log in with the appropriate credentials to access this resource.",
  },
  403: {
    message: "403 Access Forbidden",
    defaultDetails: "You don't have necessary permission to view this resource.",
  },
  500: {
    message: "Oops! Something went wrong :')",
    defaultDetails: "We apologize for the inconvenience. Please try again later.",
  },
  503: {
    message: "503 Website is under maintenance!",
    defaultDetails: "The site is not available at the moment. We'll be back online shortly.",
  },
};
...
export function ProductionErrorDisplay({ message, details }: ErrorDisplayProps) {
  return (
...
export function GeneralErrorBoundary() {
  ...
  if (isRouteErrorResponse(error)) {
    ...
    const details = error.statusText || errorConfig?.defaultDetails || defaultDetails;
  ...
  if (import.meta.env.DEV && error && error instanceof Error) {
    ...
    return <DevErrorDisplay message={defaultMessage} details={error.message} stack={error.stack} />;
  }

  // Handle other errors
  return <ProductionErrorDisplay message={defaultMessage} details={defaultDetails} />;
}
