...
import { ACCEPTED_IMAGE_TYPES, MAX_FILE_SIZE } from "~/lib/validations/settings";
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative inline-flex">
        ...
        <input {...getInputProps()} name="image" className="sr-only" aria-label="Upload image file" tabIndex={-1} />
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-140 *:[button]:hidden">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Avatar cropper dialog</DialogDescription>
          </DialogHeader>
          {previewUrl && (
            <Cropper
          ...
          <DialogFooter className="border-t px-4 py-6">
            <Button type="button" variant="outline" aria-label="Cancel" onClick={() => setIsDialogOpen(false)}>
              Cancel
            </Button>
            <Button type="button" aria-label="Set avatar" onClick={handleApply} disabled={!previewUrl} autoFocus>
              Set avatar
            </Button>
          </DialogFooter>
        </DialogContent>
